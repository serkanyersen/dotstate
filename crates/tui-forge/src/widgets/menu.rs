...
impl MenuState {
    ...
    /// Move selection to the previous item, wrapping to the last item.
    pub fn select_previous_wrap(&mut self, total: usize) {
        if total == 0 {
            return;
        }
        let sel = self.selected.unwrap_or(0);
        self.selected = Some(if sel == 0 { total - 1 } else { sel - 1 });
    }

    /// Move selection to the next item, wrapping to the first item.
    pub fn select_next_wrap(&mut self, total: usize) {
        if total == 0 {
            return;
        }
        let sel = self.selected.unwrap_or(0);
        self.selected = Some(if sel >= total - 1 { 0 } else { sel + 1 });
    }
...
impl StatefulWidget for Menu {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        for (i, item) in self.items.iter().enumerate() {
            ...
            let border_char = if is_selected { "\u{258C}" } else { " " };
...
mod tests {
    ...
    fn test_menu_item_creation() {
        let item = MenuItem::new("\u{1F4C1}", "Test Item", Color::Cyan);
        assert_eq!(item.icon, "\u{1F4C1}");
    ...
    fn test_menu_item_disabled() {
        let item = MenuItem::new("\u{1F4C1}", "Test", Color::Cyan).enabled(false);
        assert!(!item.enabled);
    }
